 ...
 * 
 * Strategi: WiFi ON/OFF
 *   - WiFi hanya aktif saat pengecekan update
 *   - WiFi dimatikan setelah selesai
 *   - Menghemat power consumption
 * 
...
enum OTAState {
  ...
  OTA_CONNECTING,     // Menghubungkan WiFi
...
private:
  ...
  bool connectWiFi() {
    if (OTA_DEBUG) Serial.println("[OTA] Menghubungkan WiFi...");
    ...
    while (WiFi.status() != WL_CONNECTED) {
      ...
      delay(500);
      if (OTA_DEBUG) Serial.print(".");
    }
    
    Serial.printf("\n[OTA] WiFi terhubung: %s\n", WiFi.localIP().toString().c_str());
    return true;
  }

  // Matikan WiFi
  void disconnectWiFi() {
    if (OTA_DEBUG) Serial.println("[OTA] Mematikan WiFi...");
    WiFi.disconnect(true);  // true = putar off radio
    WiFi.mode(WIFI_OFF);
    delay(100);  // Beri waktu untuk cleanup
    if (OTA_DEBUG) Serial.println("[OTA] WiFi dimatikan");
  }
...
public:
  ...
  void begin() {
    if (OTA_DEBUG) {
      Serial.println(F("========================================"));
      Serial.println(F("  HydroIoT OTA System"));
      Serial.println(F("========================================"));
    ...
    switch (state) {
      ...
      case OTA_CONNECTING:
        ...
        if (now - stateStartTime > OTA_TIMEOUT) {
          Serial.println("[OTA] TIMEOUT saat koneksi WiFi");
          state = OTA_ERROR;
          stateStartTime = now;
          break;
        }
